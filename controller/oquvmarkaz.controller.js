...
const {
  validateCreate,
  validateUpdate,
} = require("../validation/oquvmarkaz.validation");
...
exports.createOquvmarkaz = async (req, res) => {
  try {
    ...
    if (existing)
      return res
        .status(400)
        .json({ message: "Bu telefon raqami bilan markaz mavjud!" });

    const existingMap = await Oquvmarkaz.findOne({ where: { googlemap } });
    if (existingMap)
      return res
        .status(400)
        .json({ message: "Bu Google Map manzili bilan markaz mavjud!" });

    const newMarkaz = await Oquvmarkaz.create(req.body);
    res
      .status(201)
      .json({ message: "O‘quv markaz qo‘shildi!", data: newMarkaz });
  } catch (error) {
    res.status(500).json({ message: "Server xatosi!", error: error.message });
  }
};
...
exports.getOquvmarkazById = async (req, res) => {
  try {
    const markaz = await Oquvmarkaz.findByPk(req.params.id);
    if (!markaz) return res.status(404).json({ message: "Markaz topilmadi!" });
    res.json(markaz);
  } catch (error) {
    res.status(500).json({ message: "Server xatosi!", error: error.message });
  }
...
exports.updateOquvmarkazById = async (req, res) => {
  try {
    ...
    // Telefon raqam yoki map manzili o‘zgartirilgan bo‘lsa, tekshiramiz
    if (req.body.number !== markaz.number) {
      const existing = await Oquvmarkaz.findOne({
        where: { number: req.body.number },
      });
      if (existing)
        return res
          .status(400)
          .json({ message: "Bu telefon raqami bilan boshqa markaz mavjud!" });
    }

    if (req.body.googlemap !== markaz.googlemap) {
      const existingMap = await Oquvmarkaz.findOne({
        where: { googlemap: req.body.googlemap },
      });
      if (existingMap)
        return res.status(400).json({
          message: "Bu Google Map manzili bilan boshqa markaz mavjud!",
        });
    }

    await markaz.update(req.body);
    res.json({ message: "Markaz yangilandi!", data: markaz });
  } catch (error) {
    res.status(500).json({ message: "Server xatosi!", error: error.message });
  }
};
